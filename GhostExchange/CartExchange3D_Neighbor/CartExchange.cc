...
int main(int argc, char *argv[])
{
   ...
   int xyplane_mult = (jsize+2*nhalo)*(isize+2*nhalo)*8;
   int xstride_mult = (isize+2*nhalo)*8;
   MPI_Aint sdispls[6] = {nhalo  *xyplane_mult,         // frnt is nhalo behind front
                          ksize  *xyplane_mult,         // back is ksize behind front
                          nhalo  *xstride_mult,         // bottom row is nhalo above start
                          jsize  *xstride_mult,         // top row is jsize above start
                          nhalo  *8,                    // left column is nhalo right of start
                          isize  *8};                   // right column is isize right of start
   MPI_Aint rdispls[6] = {0,                            // frnt ghost is 0 from front
                          (ksize+nhalo)  *xyplane_mult, // back ghost is ksize+nhalo behind front
                          0,                            // bottom ghost row is 0 above start
                          (jsize+nhalo)  *xstride_mult, // top ghost row is jsize+nhalo above start
                          0,                            // left ghost column is 0 right of start
                          (isize+nhalo)*8};             // right ghost row is jsize+nhalo right of start
   MPI_Datatype sendtypes[6] = {depth_type, depth_type, vert_type, vert_type, horiz_type, horiz_type};
   ...
   ghostcell_update(x, nhalo, corners, horiz_type, vert_type, depth_type, sendtypes, recvtypes,
                    sdispls, rdispls, cart_comm, do_timing);
